...
internal sealed class ControlledDelayPromise
{
    ...
    public ControlledDelayPromise(TimeSpan delay, CancellationToken cancellationToken)
    {
        _delay = delay;
    ...
    private void Initialize()
    {
        ...
        if (_cancellationToken.CanBeCanceled)
        {
            _cancellationRegistration = _cancellationToken.UnsafeRegister(
                static (state, token) => ((ControlledDelayPromise)state!).CompleteFromCancellation(token),
                this);
...
internal sealed class ControlledTaskCompletionRace
{
    ...
    private IControlledWorkRegistration? _taskRegistration;
    private IControlledTimeout? _deadline;
    private CancellationTokenRegistration _cancellationRegistration;
    private int _winner;

    public ControlledTaskCompletionRace(
        Task task,
        TimeSpan timeout,
        Action taskWinner,
    ...
    public void Start()
    {
        _taskRegistration = ControlledTaskRuntime.ScheduleCancelableContinuation(
            _task,
            CompleteFromTask,
            Api);
        ...
        {
            _deadline = ControlledTaskRuntime.RegisterTimeout(_timeout, CompleteFromTimeout, Api);
        }

        if (_cancellationToken.CanBeCanceled)
        {
            _cancellationRegistration = _cancellationToken.UnsafeRegister(
                static state => ((ControlledTaskCompletionRace)state!).CompleteFromCancellation(),
                this);
            if (Volatile.Read(ref _winner) != 0)
            {
                _cancellationRegistration.Unregister();
