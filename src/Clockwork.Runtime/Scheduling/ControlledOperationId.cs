...
/// <summary>
/// <para>
/// A stable, process-friendly identity for one <see cref="ControlledOperation"/> within a single
/// <see cref="ControlledOperationScheduler"/>. Values are assigned by the scheduler in strictly
...
/// one operation has exactly one <see cref="ControlledOperationId"/> for its whole lifetime, even
...
public readonly record struct ControlledOperationId(long Value) : IComparable<ControlledOperationId>
{
    /// <summary>
    ...
    /// </summary>
    public static ControlledOperationId None => default;
    ...
    /// <inheritdoc />
    public int CompareTo(ControlledOperationId other) => Value.CompareTo(other.Value);
    ...
    public static bool operator <(ControlledOperationId left, ControlledOperationId right) => left.Value < right.Value;
