...
/// </summary>
public sealed class ControlledSchedulingStrategyTests
{
    ...
    private static List<long> DriveThreeYieldingOperations(ControlledOperationScheduler scheduler)
    {
        var order = new List<long>();
        ScheduleYielding(scheduler, order);
        ScheduleYielding(scheduler, order);
    ...
    private static void ScheduleYielding(ControlledOperationScheduler scheduler, List<long> order, int priority = 0)
    {
        ControlledOperation? self = null;
