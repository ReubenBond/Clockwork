...
/// </summary>
public sealed class ControlledResourceCancellationTests
{
    ...
    public void AlreadyCanceledTokenResolvesCanceledWithoutParking()
    {
        ...
        Assert.Equal(ControlledWaitOutcome.Canceled, outcome);
        Assert.True(paused);
    ...
    public void CancelingTokenWhileParkedWakesTheOperationWithCanceled()
    {
        ...
        // Park the waiter.
        Assert.True(scheduler.RunStep());
        Assert.Equal(ControlledOperationState.Paused, waiter.State);
    ...
    public void OnlyTheCanceledWaiterWakesWhenSharedTokenCancelsOneOfManyWaits()
    {
        ...
        scheduler.Drain();
        Assert.Equal(ControlledWaitOutcome.Canceled, results["a"]);
        Assert.False(results.ContainsKey("b"));

        Assert.NotNull(scheduler.SignalOne(resource));
        scheduler.Drain();
        Assert.Equal(ControlledWaitOutcome.Signaled, results["b"]);
    }
}
