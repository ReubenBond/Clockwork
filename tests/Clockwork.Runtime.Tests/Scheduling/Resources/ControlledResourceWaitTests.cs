...
/// </summary>
public sealed class ControlledResourceWaitTests
{
    ...
    public void WaitYieldsTheBatonAndSignalResumesWithSignaledOutcome()
    {
        ...
        var outcome = (ControlledWaitOutcome?)null;
        ...
        Assert.Equal(ControlledOperationState.Completed, waiter.State);
    ...
    public void NoLostWakeupsWhenManyWaitersAcrossManyResourcesAreEachSignaledOnce()
    {
        ...
        var resources = new List<ControlledResource>();
        var woke = new System.Collections.Concurrent.ConcurrentDictionary<string, int>();

        for (var r = 0; r < resourceCount; r++)
        {
            var resource = scheduler.CreateResource(ControlledResourceKind.Semaphore, $"r{r}");
    ...
    public void WaitOnResourceFromOutsideAnOperationThrows()
    {
        using var scheduler = SchedulerTestHarness.NewScheduler();
        var resource = scheduler.CreateResource(ControlledResourceKind.Custom, "r");
        Assert.Throws<ControlledOperationException>(() => scheduler.WaitOnResource(resource, Reason("r")));
    }
    ...
    public void WaitOnAResourceFromAnotherSchedulerThrows()
    {
        ...
        var foreign = other.CreateResource(ControlledResourceKind.Custom, "foreign");

        ControlledOperationException? caught = null;
        scheduler.Schedule("op", () =>
        {
            ...
            catch (ControlledOperationException ex)
            {
                caught = ex;
            }
        });
